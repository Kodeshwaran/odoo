...
class AccountReport(models.Model):
    ...
    company_id = fields.Many2one('res.company',compute="compute_company_id")
    ...
    def export_to_pdf(self, options):
        ...
        body = self.env['ir.ui.view']._render_template(
            "account_reports.report_customer_statement",
            values=rcontext
        )

        footer = self.env['ir.actions.report']._render_template(
            "web.minimal_layout",
            values=dict(rcontext, subst=True, body=("account_reports.report_customer_statement"))
        )
        ...
        file_content = self.env['ir.actions.report']._run_wkhtmltopdf(
            ...
            specific_paperformat_args={
                ...
                'data-report-margin-left': 3,
                'data-report-margin-right': 3,
                'data-report-header-spacing': 3,
                'data-report-margin-bottom': 3,
            }
        )

        return {
            'file_name': 'Customer_Statement.pdf',  # Custom file name
