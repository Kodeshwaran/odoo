...
class CrmLead(models.Model):
    ...
    def action_view_sale_quotation(self):
        ...
        quotations = self.order_ids.filtered(
            lambda o: o.opportunity_id.id == self.id and self._get_lead_quotation_domain())
        if len(quotations) == 1:
            action['views'] = [(self.env.ref('sale.view_order_form').id, 'form')]
            action['res_id'] = quotations.id
        else:
            action['views'] = [(self.env.ref('sale.view_order_tree').id, 'tree')]  # Assuming a tree view ID
