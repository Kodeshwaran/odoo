...
#    _inherit = ['mail.thread', 'ir.needaction_mixin']
    ...
    def _purchase_order_line_count(self):
        ...
        for order_line in self:
            order_line.purchase_order_line_count = purchase_order_lines_obj.search_count([('job_cost_id','=',order_line.id), ('order_id.state', 'in', ['done', 'purchase'])])
...
#        "account.invoice.line",
    ...
    def action_view_purchase_order_line(self):
        self.ensure_one()
        purchase_order_lines_obj = self.env['purchase.order.line']
        cost_ids = purchase_order_lines_obj.search([('job_cost_id','=',self.id), ('order_id.state', 'in', ['done', 'purchase'])]).ids
        action = {
