...
class HrEmployeeDocument(models.Model):
    ...
    def action_open_renewal(self):
        return {
            'name': _('Document Renew'),
            'type': 'ir.actions.act_window',
            'res_model': 'document.renew',
            'view_mode': 'form',
            'target': 'new',
            'context': {'default_name': self.name, 'default_document_id': self.id}
        }
    ...
    renewal_date = fields.Date(string='Renewal Date', default=fields.datetime.now())
    expiry_date = fields.Date(string='Expiry Date', copy=False, help="Date of expiry")
    employee_ref = fields.Many2one('hr.employee', invisible=1, default=lambda x: x.env['hr.employee'].search([('user_id', '=', x.env.uid)]), copy=False, readonly=True)
    ...
    renewal_line_ids = fields.One2many('document.renewal.line', 'renewal_id', string="Renewal")
...
class RenewalLine(models.Model):
    _name = 'document.renewal.line'

    renewal_id = fields.Many2one('hr.employee.document')
...
class DocumentRenew(models.TransientModel):
    _name = 'document.renew'

    name = fields.Char(string="Document Number")
    document_id = fields.Many2one('hr.employee.document')
    # document_renewal_id = fields.Many2one('document.renewal.line')
    renewal_date = fields.Date(string='Renewal Date', default=fields.datetime.now(), required=True)
    issue_date = fields.Date(string='Issue Date', required=True)
    expiry_date = fields.Date(string='Expiry Date', required=True)

    def update(self):
        renewal_obj = self.env['document.renewal.line']
        renewal_obj.create({
            'renewal_id': self.document_id.id,
            'name': self.document_id.name,
            'renewal_date': self.document_id.renewal_date,
            'issue_date': self.document_id.issue_date,
            'expiry_date': self.document_id.expiry_date
        })

        self.document_id.write({'renewal_date': self.renewal_date})
        self.document_id.write({'issue_date': self.issue_date})
        self.document_id.write({'expiry_date': self.expiry_date})
