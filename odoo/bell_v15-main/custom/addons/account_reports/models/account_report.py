...
class AccountReport(models.AbstractModel):
    ...
    def _get_lines(self, options, line_id=None):
        print(options, 'optionssssssssssss')
        return []
    ...
    def _get_reports_buttons(self, options):
        return [
            ...
            {'name': _('Analytic XLSX'), 'sequence': 3, 'action': 'print_aging_report', 'file_export_type': _('XLSX')},
    ...
    def print_analytic_xlsx(self, options):
        return {
    ...
    def calculate_aging(self, current_date, due_date):
        """Determine the aging bucket based on days overdue for each invoice."""
        if not due_date:
            return 0  # "As of"
        ...
        if delta_days <= 0:
            ...
            return 3
        elif 91 <= delta_days <= 120:
            return 4
        else:
            return 5  # "Older"

    def print_aging_report(self, options):
        """Fetch and generate the aging report as an XLSX file."""
        ...
        if options.get('filter_account_type') == 'receivable':
            domain.append(('account_id.user_type_id', '=', self.env.ref('account.data_account_type_receivable').id))
            domain.append(('account_id.user_type_id.include_initial_balance', '=', True))

        if options.get('filter_account_type') == 'payable':
            domain.append(('account_id.user_type_id', '=', self.env.ref('account.data_account_type_payable').id))
            domain.append(('account_id.user_type_id.include_initial_balance', '=', True))
        ...
        print(lines, 'lines\n')
        for line in lines:
            if line.move_id.id == 635864:
                print('YESSSSSSSSSSSSSS\n')
            partner_name = line.partner_id.name or "Unknown Partner"
            if partner_name not in data:
                data[partner_name] = {'lines': []}

            due_date = line.move_id.invoice_date_due if line.move_id.move_type != 'entry' else line.move_id.date
            aging_bucket = self.calculate_aging(current_date, due_date or line.move_id.date)
            total = line.amount_residual

            # Skip lines with zero total
            if total == 0:
                continue
            ...
            if line.move_id.analytic_account_id:
                analytic_name += f"[{line.move_id.analytic_account_id.code}] {line.move_id.analytic_account_id.name}"
            elif line.analytic_account_id:
                analytic_name += f"[{line.analytic_account_id.code}] {line.analytic_account_id.name}"
            else:
                analytic_name += 'N/A'

            # analytic_name =  (
            #     f"[{line.move_id.analytic_account_id.code}] {line.move_id.analytic_account_id.name}"
            #     if line.move_id.analytic_account_id
            #     line.analytic_account_id
            #     elif line.analytic_account_id
            #     else 'N/A'
            # )
            data[partner_name]['lines'].append({
                'line': line,
                'aging': aging_bucket,
                'invoice_number': line.move_id.name,
                'date_due': due_date,
                'account': f"{line.account_id.code} {line.account_id.name}" if line.account_id else 'N/A',
                'total': total,
        ...
        # Set column widths
        sheet.set_column(0, 12, 20)  # Adjust all columns equally

        headers = [
            'Partner', 'Invoice Number', 'Account', 'Due Date', 'Analytic Account', 'Expected Date',
            f'As of: {as_of_date}', '1 - 30', '31 - 60', '61 - 90', '91 - 120', 'Older', 'Total'
        ]
        ...
        for partner_name, partner_data in data.items():
            # print(sum(item['total'] for item in partner_data['lines']), 'total lines\n')
            # if any(item['line'].move_id.move_type != 'entry' for item in partner_data['lines']):
            print(partner_name, 'Partner Name\n')
            print(partner_data, 'Partner Data\n')
            sheet.write(row_num, 0, f"Partner: {partner_name}", title_style)
            row_num += 1

            for item in partner_data['lines']:
                aging_bucket = item['aging']

                # Write individual invoice details
                sheet.write(row_num, 1, item['invoice_number'])
                sheet.write(row_num, 2, item['account'])
                sheet.write(row_num, 3, item['date_due'], date_style)
                sheet.write(row_num, 4, item['analytic_account'])
                sheet.write(row_num, 5, item['line'].date, date_style)

                # Write aging buckets
                for bucket, col_num in {0: 6, 1: 7, 2: 8, 3: 9, 4: 10, 5: 11}.items():
                    if bucket == aging_bucket:
                        sheet.write(row_num, col_num, item['total'], default_style)
                    else:
                        sheet.write(row_num, col_num, 0, default_style)

                sheet.write(row_num, 12, item['total'], default_style)
                row_num += 1
