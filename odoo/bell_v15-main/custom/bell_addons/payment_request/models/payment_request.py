...
class PaymentRequest(models.Model):
    ...
    payment_request_lines = fields.One2many('payment.request.line', 'request_id', string='Payment Request Lines', copy=False)
    payment_request_date = fields.Date(string='Date', default=lambda self: self._get_local_today())
    ...
    def _get_local_today(self):
        # Get user's timezone-aware datetime
        tz_datetime = fields.Datetime.context_timestamp(self, datetime.now())
        return tz_datetime.date()

    @api.depends('payment_request_lines.amount_pay')
    def _compute_amount_pay(self):
        for rec in self:
            rec.amount_pay = sum(line.amount_pay or 0.0 for line in rec.payment_request_lines)
    ...
    def action_send_for_approval(self):
        if not self.journal_id:
            raise ValidationError('Kindly select the Journal')
        for line in self.payment_request_lines:
            if line.amount_pay == 0:
                raise ValidationError(_('%s has amount to pay as 0. Kindly enter value greater than 0') % line.move_id.name)
    ...
    def action_register_payment(self):
        ...
        for line in self.payment_request_lines:
            payment_register = self.env['account.payment.register'].with_context({'active_model': 'account.move', 'active_ids': line.move_id.id}).create({'journal_id': self.journal_id.id, 'amount': line.amount_pay,'request_id': self.id})
            payment_request_lines.append(payment_register.id)
        return {
            ...
            'context': {
                ...
                'default_payment_request_lines': [(6, 0, payment_request_lines)],
            },
            'target': 'new',
...
class PaymentRequestLine(models.Model):
    ...
    payment_state = fields.Selection([('paid', 'Paid'), ('not_paid', 'Not Paid')], string='Payment Status', related='move_id.payment_state')
    state = fields.Selection([('draft', 'Draft'), ('posted', 'Posted')], string='Status', related='move_id.state')
    ref = fields.Char(string='Bill Reference', related='move_id.ref')
    company_currency_id = fields.Many2one(string='Company Currency', related='move_id.company_currency_id')
    amount_untaxed = fields.Monetary(string='Untaxed Amount', related='move_id.amount_untaxed', currency_field='company_currency_id')
    amount_total = fields.Monetary(string='Total', related='move_id.amount_total', currency_field='company_currency_id')
    amount_residual = fields.Monetary(string='Amount Due', related='move_id.amount_residual', currency_field='company_currency_id')
    amount_pay = fields.Monetary(
    ...
    def _onchange_move_id_set_amount_pay(self):
        for rec in self:
            # Only auto-set if user hasn't entered anything yet
            if rec.move_id and not rec.amount_pay:
                rec.amount_pay = rec.move_id.amount_residual

    @api.onchange('amount_pay')
    def _onchange_amount_to_pay(self):
        for rec in self:
            if rec.amount_pay and rec.move_id and rec.amount_pay > rec.move_id.amount_residual:
                raise ValidationError('You cannot enter an amount greater than the due amount')

    @api.model
    def create(self, vals):
        if vals.get('move_id') and not vals.get('amount_pay'):
            move = self.env['account.move'].browse(vals['move_id'])
            vals['amount_pay'] = move.amount_residual
...
class AccountPaymentRegister(models.TransientModel):
    ...
    def _get_wizard_values_from_batch(self, batch_result):
        ...
        print("--------", res, "----res---\n")
        if self.request_id:
            res['source_amount'] = sum(self.request_id.payment_request_lines.filtered(lambda x: x.move_id.id in batch_result['lines'].mapped('move_id').ids).mapped('amount_pay'))
            res['source_amount_currency'] = sum(self.request_id.payment_request_lines.filtered(lambda x: x.move_id.id in batch_result['lines'].mapped('move_id').ids).mapped('amount_pay'))
        print("--------", res,"----res---\n")
        return res
    ...
    def _create_payment_vals_from_wizard(self):
        ...
        if self.request_id:
            payment_vals['request_id'] = self.request_id.id
        return payment_vals
