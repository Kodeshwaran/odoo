...
class AccountMove(models.Model):
    ...
    def payment_request(self):
        ...
        payment_request = self.env['payment.request'].create({'account_move_ids': self.ids})
        for bill in self.ids:
            bill_values = {
                'move_id': bill
            }
            payment_request.write({'payment_request_lines': [(0, 0, bill_values)]})
        return True
