...
class Lead2OpportunityPartner(models.TransientModel):
    ...
    def action_apply(self):
        ...
        self.env.cr.execute('''update crm_lead set is_opportunity=True where id=%d'''%(stage.id))
        if self.partner_id:
            s=[]
            values['partner_id'] = self.partner_id.id
            values['is_opportunity'] = True
            stage = self.env['crm.lead'].browse(self._context.get('active_ids', []))
            if not stage.contact:
                ...
                dfg.update({'contact':s,'is_opportunity':True})
        if self.name == 'merge':
            values['is_opportunity']= True
            leads = self.opportunity_ids.merge_opportunity()
            if leads.type == "lead":
                values.update({'lead_ids': leads.ids, 'user_ids': [self.user_id.id], 'is_opportunity':True})
