...
class AccountMove(models.Model):
    ...
    invoice_grn_status = fields.Selection([
        ('grn_done', 'GRN Done'),
        ('grn_undone', 'GRN Undone')
    ], default='grn_undone', string="GRN Invoice Status")

    @api.onchange('invoice_origin')
    def _onchange_sale_id(self):
        for move in self:
            # Avoid recursion by checking the context flag
            if not self.env.context.get('is_updating_grn_status', False):
                # Set the flag in context to avoid recursion
                context = dict(self.env.context, is_updating_grn_status=True)
                self = self.with_context(context)

                # Process the logic only if invoice_origin is set
                if move.invoice_origin:
                    sale_order = self.env['sale.order'].search([('name', '=', move.invoice_origin)], limit=1)
                    if sale_order:
                        if sale_order.tender_id:
                            # Get the purchase order linked to the tender_id
                            purchase_order = self.env['purchase.order'].search(
                                [('requisition_id', '=', sale_order.tender_id.id)], limit=1)
                            if purchase_order:
                                # Check if there is a related stock.picking (delivery document) and its status is 'done'
                                picking = self.env['stock.picking'].search(
                                    [('purchase_id', '=', purchase_order.id), ('state', '=', 'done')], limit=1)
                                if picking:
                                    move.invoice_grn_status = 'grn_done'  # Set to 'GRN Done' if delivery is done
                                else:
                                    move.invoice_grn_status = 'grn_undone'  # Set to 'GRN Undone' if no delivery or not done
                            else:
                                move.invoice_grn_status = 'grn_undone'  # Set to 'GRN Undone' if no purchase order linked
                        else:
                            move.invoice_grn_status = 'grn_undone'  # Set to 'GRN Undone' if no tender_id
                    else:
                        move.invoice_grn_status = 'grn_undone'  # Set to 'GRN Undone' if no sale order
                else:
                    move.invoice_grn_status = 'grn_undone'  # Set to 'GRN Undone' if no sale order link

    # Override the create method to avoid recursion using context
    @api.model
    def create(self, vals):
        ...
        if 'invoice_origin' in vals:
            ...
            move._onchange_sale_id()  # Ensure it updates based on invoice_origin

        return move

    # Override the write method to avoid recursion using context
    def write(self, vals):
        if 'invoice_grn_status' in vals:
            # Avoid recursion by checking the context flag
            if not self.env.context.get('is_updating_grn_status', False):
                context = dict(self.env.context, is_updating_grn_status=True)
                self = self.with_context(context)
                res = super(AccountMove, self).write(vals)
                self._onchange_sale_id()  # Ensure GRN status is updated if invoice_origin is changed
                return res
        return super(AccountMove, self).write(vals)

    # Method to update GRN status for existing records
    @api.model
    def init(self):
        if 'sale.order' in self.env:
            moves = self.search([])  # Retrieve all records
            for move in moves:
                if move.invoice_origin:
                    sale_order = self.env['sale.order'].search([('name', '=', move.invoice_origin)], limit=1)
                    if sale_order:
                    ...
                    move.invoice_grn_status = 'grn_undone'
